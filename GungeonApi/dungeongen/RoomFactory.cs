...
namespace GungeonAPI
{
    public static class RoomFactory
    {
        ...
        {
            var roomsData = BuildFromResource("ClassLibrary1/GungeonApi/resource/rooms/damnedtemple.room");
            

                if (!RoomFactory.rooms.ContainsKey(roomsData.room.name))
                {
                    //Tools.PrintNoID($"Found room: \"{name}\"");
                    rooms.Add(roomsData.room.name, roomsData);
                    RoRItems.Log($"{roomsData.room.name} started successfully.", RoRItems.TEXT_COLOR);
                }
