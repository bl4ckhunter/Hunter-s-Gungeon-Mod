...
namespace GungeonAPI
{
	...
	public class ExitWayShrineInteractible : SimpleInteractable, IPlayerInteractable
	{
		...
		{   
			...
			ExitWayShrineInteractible.HoleObject = PickupObjectDatabase.GetById(155).GetComponent<SpawnObjectPlayerItem>();
			ExitWayShrineInteractible hellcomponent = gameObject.GetComponent<ExitWayShrineInteractible>();
			...
			hellcomponent.synergyobject = ExitWayShrineInteractible.HoleObject.objectToSpawn;
		...
		private IEnumerator HandleConversation(PlayerController interactor)
		{
			...
			TextBoxManager.ShowTextBox(this.talkPoint.position, this.talkPoint, -1f, "Do you wish to leave? If the challenge hasn't been completed there will be no return.", interactor.characterAudioSpeechTag, false, TextBoxManager.BoxSlideOrientation.NO_ADJUSTMENT, true, false);
			string acceptanceTextToUse = this.acceptText;
			string declineTextToUse = this.declineText;
			GameUIRoot.Instance.DisplayPlayerConversationOptions(interactor, null, "<Leave>", "<Walk Away>");
