...
namespace Swordtress
{
    public class SlashDoer
    {
        ...
        public static void DoSwordSlash( Vector2 position, float angle, PlayerController owner, float playerKnockbackForce, ProjInteractMode intmode, float damageToDeal, float enemyKnockbackForce, List<GameActorEffect> statusEffects, Transform parentTransform = null, float jammedDamageMult = 1, float bossDamageMult = 1, float SlashRange = 2.5f, float SlashDimensions = 90f, bool DoBlood = true)
        {
            GameManager.Instance.StartCoroutine(HandleSlash(position, angle, owner, playerKnockbackForce, intmode, damageToDeal, enemyKnockbackForce, statusEffects, jammedDamageMult, bossDamageMult, SlashRange, SlashDimensions, DoBlood));
        }
        private static IEnumerator HandleSlash( Vector2 position, float angle, PlayerController owner, float knockbackForce, ProjInteractMode intmode, float damageToDeal, float enemyKnockback, List<GameActorEffect> statusEffects, float jammedDMGMult, float bossDMGMult, float SlashRange, float SlashDimensions, bool DoBlood)
        {
            ...
            while (ela < 0.2f)
            {
                ...
                HandleHeroSwordSlash( alreadyHit, position, angle, slashId, owner, intmode, damageToDeal, enemyKnockback, statusEffects, jammedDMGMult, bossDMGMult, SlashRange, SlashDimensions, DoBlood);
                yield return null;
            }
        ...
        private static void HandleHeroSwordSlash(List<SpeculativeRigidbody> alreadyHit, Vector2 arcOrigin, float slashAngle, int slashId, PlayerController owner, ProjInteractMode intmode, float damageToDeal, float enemyKnockback, List<GameActorEffect> statusEffects, float jammedDMGMult, float bossDMGMult, float slashRange, float slashDimensions, bool doBlood)
        {
            ...
            DealDamageToEnemiesInArc(owner, arcOrigin, slashAngle, slashRange, damageToDeal, enemyKnockback, statusEffects, jammedDMGMult, bossDMGMult, slashDimensions, doBlood, alreadyHit);
                        ...
                        for (int j = 0; j < aiactor.healthHaver.NumBodyRigidbodies; j++)
                        {
                            ...
                            if (hitboxPixelCollider != null)
                            {
                                ...
                                if (ObjectWasHitBySlash(vector, arcOrigin, arcAngle, arcRadius, slashDimensions))
                                {
                                    ...
                                    if (flag)
                                    {
                                        float damage = DealSwordDamageToEnemy(owner, aiactor, arcOrigin, vector, arcAngle, overrideDamage, overrideForce, statusEffects, bossDMGMult, jammedDMGMult, doBlood);
                                        if (alreadyHit != null)
                                        {
        ...
        private static float DealSwordDamageToEnemy(PlayerController owner, AIActor targetEnemy, Vector2 arcOrigin, Vector2 contact, float angle, float damage, float knockback, List<GameActorEffect> statusEffects, float bossDMGMult, float jammedDMGMult, bool doBlood)
        {
            if (targetEnemy.healthHaver)
            {
                ...
                if (doBlood)
                {
                    ...
                    UnityEngine.Object.Instantiate<GameObject>(teleporter.TelefragVFXPrefab, targetEnemy.CenterPosition, Quaternion.identity);
                    UnityEngine.Object.Instantiate<GameObject>(teleporter.TelefragVFXPrefab, targetEnemy.CenterPosition, Quaternion.identity);
                    UnityEngine.Object.Instantiate<GameObject>(teleporter.TelefragVFXPrefab, targetEnemy.CenterPosition, Quaternion.identity);
                }
