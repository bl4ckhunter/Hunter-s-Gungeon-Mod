...
namespace Items
{
	...
	internal class HollowScroll : PlayerItem
	{
		...
		{
			string name = "Hollow escape route";
			string resourceName = "ClassLibrary1/Resources/Hollow";
			GameObject gameObject = new GameObject();
			HollowScroll HollowScroll = gameObject.AddComponent<HollowScroll>();
			ItemBuilder.AddSpriteToObject(name, resourceName, gameObject);
			string shortDesc = "To the Hollow!";
			string longDesc = "Use it after killing a boss to descend to the Hollow from an higher floor.\n\n" +  "The Old King knew of chancellor Toadstool's treasonous intents and drafted a few escape plans, though he did share both his and Toadstool's plans with the current Bullet King the latter discarded them thoughtlessly, in time he'll regret doing that. \n";
			HollowScroll.SetupItem(shortDesc, longDesc, "ror");
			HollowScroll.AddPassiveStatModifier(PlayerStats.StatType.AdditionalItemCapacity, 1f, StatModifier.ModifyMethod.ADDITIVE);
			HollowScroll.quality = PickupObject.ItemQuality.EXCLUDED;
		}
		...
		{
			this.flag = false;
			if (GameManager.Instance.Dungeon.tileIndices.tilesetId == GlobalDungeonData.ValidTilesets.CATACOMBGEON || GameManager.Instance.Dungeon.tileIndices.tilesetId == GlobalDungeonData.ValidTilesets.OFFICEGEON || GameManager.Instance.Dungeon.tileIndices.tilesetId == GlobalDungeonData.ValidTilesets.FORGEGEON || GameManager.Instance.Dungeon.tileIndices.tilesetId == GlobalDungeonData.ValidTilesets.HELLGEON)
