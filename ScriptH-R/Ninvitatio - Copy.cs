...
namespace Items
{
	...
	public class Nchallange : PlayerItem
	{
		...
		protected override void DoEffect(PlayerController user)
		{
			...
			if (flag)
			{ GameManager.Instance.StartCoroutine(ReturnWarp(user));
		...
		{
			return user.CurrentRoom.GetActiveEnemiesCount(RoomHandler.ActiveEnemyType.All) <= 0; } 
		...
		public IEnumerator DoOrDie(PlayerController user)
		{
			...
			{
				this.StunEnemiesForTeleport(currentRoom, 4f);
			}
			user.ForceStopDodgeRoll();
			this.DoTentacleVFX(user);
			IntVector2? targetCenter1 = new IntVector2?(user.CenterPosition.ToIntVector2(VectorConversions.Floor));
