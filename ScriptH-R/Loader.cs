...
namespace Items
{
	...
	internal class LoaderFram : PassiveItem
	{
		// Token: 0x060001CF RID: 463 RVA: 0x0000E384 File Offset: 0x0000C584
		public static void Init()
		{
			string name = "Heavy L.O.A.D.E.R. Chassis";
			string resourceName = "ClassLibrary1/Resources/LOADER"; ;
			GameObject gameObject = new GameObject();
			LoaderFram LoaderFrame = gameObject.AddComponent<LoaderFram>();
			ItemBuilder.AddSpriteToObject(name, resourceName, gameObject);
			string shortDesc = "Steel Brawler";
			string longDesc = "Originally designed for loading dock operations this exoskeleton has been heavily modified for close combat use, the enhanced strenght and speed and the shield generator module of this suit have been enough to allow the owner to survive through a crash landing and being stuck in a time loop on an hostile planet, surely it'll serve you well in the gungeon too.  \n\n" + "Equip boxing gloves or casey to activate, hitting enemies with your equipped weapon has a chance to spawn a guon stone that will be destroyed in one hit, doing a small blank effect in the process.";
			LoaderFrame.SetupItem(shortDesc, longDesc, "ror");
			LoaderFrame.quality = PickupObject.ItemQuality.B;
			LoaderFrame.AddToSubShop(ItemBuilder.ShopType.Trorc, 1f);
			LoaderFrame.AddToSubShop(ItemBuilder.ShopType.Cursula, 1f);
			LoaderFrame.AddToSubShop(ItemBuilder.ShopType.OldRed, 1f);
			LoaderFrame.AddToSubShop(ItemBuilder.ShopType.Flynt, 1f);
			LoaderFrame.AddToSubShop(ItemBuilder.ShopType.Goopton, 1f);
		...
		public override void Pickup(PlayerController player)
		{
			...
			if (!this.used)
			{
				LootEngine.GivePrefabToPlayer(PickupObjectDatabase.GetById(539).gameObject, base.Owner);
				this.used = true;
			}
		}
		...
		private IEnumerator HandleAfterImageStop(PlayerController player)
		{
			...
			while (this.flag = base.Owner.CurrentGun.PickupObjectId == 539 ||  base.Owner.CurrentGun.PickupObjectId == 541)
			{
				yield return null;
			}
		...
		{
			this.flag = base.Owner.CurrentGun.PickupObjectId == 539 || base.Owner.CurrentGun.PickupObjectId == 541 ;
			if (this.flag && !this.active && base.Owner.CurrentGun != null)
			{
				if (!this.used1)
				{
					PickupObject pickupObject = Gungeon.Game.Items["ror:loader's_fist"];
					base.Owner.AcquirePassiveItemPrefabDirectly(pickupObject as PassiveItem);
					this.used1 = true;
				}
				GameObject gameObject = UnityEngine.Object.Instantiate<GameObject>(ResourceCache.Acquire("Global Prefabs/HoveringGun") as GameObject, base.Owner.CenterPosition.ToVector3ZisY(-5f), Quaternion.identity);

				gameObject.transform.parent = base.Owner.transform;
				this.m_hovers = gameObject.AddComponent<LoaderFram.FakeDualWield>();
				m_hovers.Aim = LoaderFram.FakeDualWield.AimType.PLAYER_AIM;
				m_hovers.Trigger = LoaderFram.FakeDualWield.FireType.ON_FIRED_GUN;
				m_hovers.Position = LoaderFram.FakeDualWield.HoverPosition.OVERHEAD;
				Gun gun;
				gun = (PickupObjectDatabase.GetById(539) as Gun);
				...
				this.m_temporaryModifier.statToBoost = PlayerStats.StatType.ChargeAmountMultiplier;
				this.m_temporaryModifier.amount = 5000000000000000f;
				this.m_temporaryModifier.modifyType = StatModifier.ModifyMethod.MULTIPLICATIVE;
				this.m_temporaryModifier5 = new StatModifier();
				this.m_temporaryModifier5.statToBoost = PlayerStats.StatType.Damage;
				this.m_temporaryModifier5.amount = 1.35f;
				this.m_temporaryModifier5.modifyType = StatModifier.ModifyMethod.MULTIPLICATIVE;
				this.m_temporaryModifier1 = new StatModifier();
				this.m_temporaryModifier1.statToBoost = PlayerStats.StatType.MovementSpeed;
				this.m_temporaryModifier1.amount = 1.20f;
				...
				this.m_temporaryModifier2.amount = 1.05f;
				...
				this.m_temporaryModifier3.amount = 1.3f;
				this.m_temporaryModifier3.modifyType = StatModifier.ModifyMethod.MULTIPLICATIVE;
				this.m_temporaryModifierb = new StatModifier();
				this.m_temporaryModifierb.statToBoost = PlayerStats.StatType.DamageToBosses;
				this.m_temporaryModifierb.amount = 2f;
				this.m_temporaryModifierb.modifyType = StatModifier.ModifyMethod.MULTIPLICATIVE;
				base.Owner.ownerlessStatModifiers.Add(this.m_temporaryModifierb);
				...
				base.Owner.ownerlessStatModifiers.Add(this.m_temporaryModifier5);
			...
			if (!this.flag)
			{
				...
				this.m_buffedTarget.ownerlessStatModifiers.Remove(this.m_temporaryModifierb);
				...
				this.m_buffedTarget.ownerlessStatModifiers.Remove(this.m_temporaryModifier5);
				base.Owner.stats.RecalculateStats(base.Owner, false, false);
		...
		{
			base.Update();
			bool flag = base.Owner.CurrentGun.PickupObjectId == 539 || base.Owner.CurrentGun.PickupObjectId == 541;
			if(!flag && this.m_hovers.gameObject != null)
		...
		private void Mimicgun(PlayerController player, float arg2, bool fatal, HealthHaver ai)
		{
			...
			this.flag = base.Owner.CurrentGun.PickupObjectId == 541 || base.Owner.CurrentGun.PickupObjectId == 539;
			if (this.flag)
			{
				if (UnityEngine.Random.value < 0.15f && !this.safetycooldown)
				{
		...
		public class FakeDualWield : BraveBehaviour, IPlayerOrbital
		{
			...
			private void UpdatePosition()
			{
				...
				{
					this.m_parentTransform.position = (this.m_owner.specRigidbody.UnitCenterRight + new Vector2(0f, 0f)).ToVector3ZisY(0f);
					base.sprite.HeightOffGround = 1.5f;
					base.sprite.UpdateZDepth();
				}
				if (this.m_owner.CurrentGun.sprite.WorldCenter.x - this.m_owner.specRigidbody.UnitCenter.x >= 0f)
				{
					this.m_parentTransform.position = (this.m_owner.specRigidbody.UnitCenterLeft + new Vector2(0f, -0.3f)).ToVector3ZisY(0f);
					base.sprite.HeightOffGround = 1.5f;
					base.sprite.UpdateZDepth();
				}
			}
